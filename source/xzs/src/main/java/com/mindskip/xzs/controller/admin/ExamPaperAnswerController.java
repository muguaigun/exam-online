...
public class ExamPaperAnswerController extends BaseApiController {
    ...
    @Autowired
    public ExamPaperAnswerController(ExamPaperAnswerService examPaperAnswerService,  UserService userService,ExamPaperService examPaperService, SubjectService subjectService, ApplicationEventPublisher eventPublisher) {
    ...
    public RestResponse answerSubmit(@RequestBody @Valid ExamPaperSubmitVM examPaperSubmitVM) {
        ...
        if (null == examPaperAnswerInfo) {
            return RestResponse.fail(2, "试卷不能重复批改");
        }
        ExamPaperAnswer examPaperAnswer = examPaperAnswerInfo.getExamPaperAnswer();
        Integer userScore = examPaperAnswer.getUserScore();
        String scoreVm = ExamUtil.scoreToVM(userScore);
        UserEventLog userEventLog = new UserEventLog(user.getId(), user.getUserName(), user.getRealName(), new Date());
        String content = user.getUserName() + " 试卷批改：" + examPaperAnswerInfo.getExamPaper().getName()
                + " 得分：" + scoreVm
